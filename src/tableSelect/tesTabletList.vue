      ...
      <van-checkbox-group v-model="result" ref="checkboxGroup">
        <template>
          ...
          <table ref="table" class="table">
            ...
            <!-- <van-sticky :container="container">
              <tr class="top">
                <th>
                  <van-checkbox 
                  v-model="mainCheck"
                  shape="square" ref="checkboxes" @change="toggle" @click="onClick"/>
                </th>
                <th v-for="(item_th, index_th) in thList" :key="index_th">
                  <span class="title">{{ item_th.title }}</span>
                </th>
            ...
            >
              <td>
                <van-checkbox shape="square" :name="item_tr.comname2" />
              </td>
...
export default {
  ...
  methods: {
    ...
    onClick(e) {
      ...
      // console.log('## target => ', e.target)
      // checkboxes
      // console.log('## this.this.$refs.checkboxGroup ==> ',this.mainCheck, this.$refs.checkboxGroup)
      // this.$refs.checkboxGroup.toggleAll(!this.mainCheck);
    },
    onLoad() {
      // 异步更新数据
      ...
      setTimeout(() => {
        for (let i = 0; i < 10; i++) {
          ...
          this.data11.push({
            comname2: "comname2" + this.data11.length + 1,
            addAgent: "addAgent",
            addAgentRate: "addAgentRate",
          });
        }
